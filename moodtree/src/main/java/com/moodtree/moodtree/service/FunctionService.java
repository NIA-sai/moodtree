...
public class FunctionService
{
	...
	public Result imageFromResponseResult ( ResponseResult responseResult )//获取生成的图片
	{
		try
		{
			if ( responseResult != null && responseResult.getStatus () == 0 )
			{
				...
				for ( ResponseResult.ContentResult.Part part : responseResult.getResult ().getOutput () )
				{
					System.out.println ( part.getStatus () );
					for ( ResponseResult.ContentResult.Content content : part.getContent () )
					{
						if ( "image".equals ( content.getType () ) )
						{
							// 获取图片URL
							System.out.println ( "start get image" );
							List < ResponseResult.ContentResult.Image > images = content.getImage ();
							for ( ResponseResult.ContentResult.Image image : images )
							{
	...
	public Result integratedAnswer ( String mood )
	{
		...
		{
			System.out.println ( e.toString () );
			System.out.println ( "in integratedAnswer" );
